...
import { X, ChevronLeft, ChevronRight, Grid3X3, MapPin } from "lucide-react";
...
interface HotelBentoGalleryProps {
  images: GalleryImage[];
}

export function HotelBentoGallery({ images }: HotelBentoGalleryProps) {
  ...
  // Ensure we have at least 5 images for the bento layout, repeat if necessary
  const displayImages = images.length >= 5 ? images : [...images, ...images, ...images, ...images, ...images].slice(0, 5);
      ...
      <div className="grid grid-cols-4 grid-rows-2 gap-2 h-[300px] md:h-[400px] rounded-xl overflow-hidden">
        ...
        >
          ...
          <Button
            variant="secondary"
            size="sm"
            className="absolute bottom-3 right-3 bg-background/95 backdrop-blur-sm shadow-lg text-foreground hover:bg-background gap-1.5 text-xs font-medium"
            onClick={(e) => {
              e.stopPropagation();
              openLightbox(0);
            }}
            ...
            <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex gap-2 max-w-[90vw] overflow-x-auto pb-2">
              {images.map((image, index) => (
                <button
                  ...
                  className={`w-16 h-12 rounded-md overflow-hidden transition-all shrink-0 ${
                    index === activeIndex
